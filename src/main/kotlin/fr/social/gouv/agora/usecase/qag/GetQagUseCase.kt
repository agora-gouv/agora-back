    ...
    private fun buildQag(qagId: String): Qag? {
        return qagInfoRepository.getQagInfo(qagId)?.let { qagInfo ->
            thematiqueRepository.getThematique(qagInfo.thematiqueId)?.let { thematique ->
                val feedbackList = getFeedbackQagRepository.getFeedbackQagList(qagId = qagId)
                Qag(
                    id = qagInfo.id,
                    ...
                    feedback = feedbackList.any { feedbackQag -> feedbackQag.userId == userId },
                    feedbackResults = if (featureFlagsUseCase.isFeatureEnabled(AgoraFeature.FeedbackResponseQag)) feedbackList else null,
                )
            }
        }.also { qag ->
            when (qag) {
