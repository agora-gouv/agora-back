...
import fr.social.gouv.agora.usecase.reponseConsultation.GetConsultationResultsWithDemographicInfoUseCase
...
class GetConsultationResponsesWithDemographicInfoController(
    private val consultationResultsUseCase: GetConsultationResultsWithDemographicInfoUseCase,
) {
    ...
    fun getConsultationResults(@PathVariable consultationId: String): ResponseEntity<*> {
        return consultationResultsUseCase.getConsultationResults(consultationId)?.let { consultationResult ->
            val header = buildHeader(consultationResult)
            val responses = buildConsultationResponsesBloc(consultationResult)
            val demographicGlobal = buildDemographicInfoBloc(consultationResult.demographicInfo)
